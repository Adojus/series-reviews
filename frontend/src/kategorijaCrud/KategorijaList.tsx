...
import { KategorijaForL } from './models';
...
class State
{
	...
	entities : KategorijaForL[] = [];

	isDeleting : boolean = false;
	entToDel : KategorijaForL | null = null;
...
/**
 * List the instances of the kategorija.
 * @returns Component HTML.
 */
function KategorijaList() {
	...
	if( !state.isInitialized || location.state === "refresh" ) {
		//query data
		backend.get<KategorijaForL[]>(
			config.backendUrl + "/kategorija/list"
		)
		.then(resp => {
			update(state => {
	 ...
	 * @param id ID of the kategorija to edit.
	 */
	...
	let onDelete = () => {
		update(() => {
			...
			backend.get(
				config.backendUrl + "/kategorija/delete",
				{
					params : {
			...
			//success
			.then(resp => {
				//force reloading of kategorija list
			...
			.catch(err => {
				...
				let msg = 
					`Deletion of kategorija '${state.entToDel!.id}' has failed. ` +
					`either kategorija is not deletable or there was backend failure.`;
				notifyFailure(msg);
			})
		});
	}
	...
	let html = 
		...
		{ state.isLoaded &&
			...
			<Dialog
				...
				header={<span className="me-2">Confirm kategorija deletion, please.</span>}
				style={{width: "50ch"}}
				>
				<div className="alert alert-warning">Do you really want to delete the following kategorija?</div>
